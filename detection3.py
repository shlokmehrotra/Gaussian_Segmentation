...
#         height1 = height
        #
        return(0, 0, 1, boxsize, boxsize, 3, 3, 1, boxsize-1, boxsize+1, 3, 2)
    ...
    Xin, Yin = np.mgrid[0:(2*boxsize), 0:(2*boxsize)]
    ...
    for i in range(2*boxsize):
        for j in range(2*boxsize):
    ...
    diffarray = diffarray.reshape(2*boxsize,2*boxsize)
...
# =============================================================================
    ...
    x = np.repeat(np.arange(2*boxsize),2*boxsize)
    y = np.tile(np.arange(2*boxsize),2*boxsize)
    #print(x)
    z = data
    
    X, Y, Z = (u.reshape(2*boxsize, 2*boxsize) for u in (x, y, z))
...
# =============================================================================
    ...
    def fitgaussian(data):
        ...
        return p
    # Create the gaussian data
    Xin, Yin = np.mgrid[0:(boxsize*2), 0:(boxsize*2)]
    ...
    diffarray = diffarray.reshape(boxsize*2,boxsize*2)
...
# =============================================================================
    ...
    x = np.repeat(np.arange(boxsize*2),boxsize*2)
    y = np.tile(np.arange(boxsize*2),boxsize*2)
    #print(x)
    z = data
    
    X, Y, Z = (u.reshape(boxsize*2, boxsize*2) for u in (x, y, z))
...
def real2SpotLoc():
    ...
    for i in range(len(newdata)):
        ...
        img = cv2.resize(img, (boxsize*2*scale, boxsize*2*scale), interpolation = cv2.INTER_NEAREST)
